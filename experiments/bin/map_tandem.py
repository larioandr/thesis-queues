...
def load_data(file_name, create_dists: bool = False, tol=.05) -> pd.DataFrame:
    ...
    def load_arrival(row):
        try:
            return MarkovArrival(row[MAP_D0_COL], row[MAP_D1_COL], tol=tol)
        except MatrixError:
            pass
        return None
    ...
    def load_ph(row):
        try:
            return PhaseType(row[PH_S_COL], row[PH_P_COL], tol=tol)
        except MatrixError:
            pass
        return None

    if create_dists:
        df['arrival'] = df.apply(load_arrival, axis=1)
        df['service'] = df.apply(load_ph, axis=1)
        df1 = df.dropna(subset=['arrival', 'service'])
        if len(df1) < len(df):
            print(f"Dropped {len(df) - len(df1)} bad rows")
    return df
